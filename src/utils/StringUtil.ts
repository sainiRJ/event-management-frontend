import {NullableString, UndefinableString} from "@/customTypes/CommonTypes";
...
function acceptingLeadingZeros(str: string): string {
	return str.replace(/^0+(?=[1-9])/, "");
}
/**
 ...
 * @returns
 */
function sanitizeNull(input: NullableString): string {
	return input || "";
}
