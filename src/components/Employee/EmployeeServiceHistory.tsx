...
const EmployeeServiceHistory: React.FC = () => {
	...
	const handleServiceSelection = (assignedEmployeeId: string) => {
		setFormData((prev) => {
			const alreadySelected = prev.assignedEmployeeIds?.includes(assignedEmployeeId);
			return {
				...prev,
	...
	const filteredServices = serviceHistory?.assignedServices.filter((service) => {
			...
			<div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
				<div className="flex items-center gap-6">
					...
					<div>
						...
						<p className="text-gray-500 font-bold mt-1">{serviceHistory?.employeeName || "Employee Record"}</p>
					</div>
				</div>
				...
				<div className="overflow-x-auto rounded-2xl border border-gray-100">
					<table className="min-w-full">
						<thead className="bg-gray-50/50">
							<tr>
								<th className="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest">Service Info</th>
			...
			>
				<div className="space-y-6">
					<div className="bg-indigo-50 rounded-2xl p-4 flex justify-between items-center">
						<span className="text-sm font-bold text-indigo-600">Total Due:</span>
						<span className="text-lg font-black text-indigo-900">{formatCurrency(formData.amount)}</span>
					</div>
					...
					<div>
						<label className="block text-sm font-bold text-gray-700 mb-2">Selected Services</label>
						<div className="max-h-48 overflow-y-auto border border-gray-100 rounded-2xl bg-gray-50/50 p-2">
							{getSelectedEmployeeServices().map((service) => (
								<div
									...
									onClick={() => handleServiceSelection(service.assignedEmployeeId)}
									className={`px-4 py-3 rounded-xl cursor-pointer flex items-center justify-between mb-1 last:mb-0 transition-all ${
										formData.assignedEmployeeIds?.includes(service.assignedEmployeeId)
											? "bg-indigo-600 text-white shadow-lg shadow-indigo-200"
											: "bg-white text-gray-600 hover:bg-gray-100"
									}`}
								>
									<span className="text-xs font-bold">{service.serviceName}</span>
					...
					<label className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl cursor-pointer hover:bg-gray-100 transition-colors border border-gray-100">
						<input
							...
							onChange={(e) => setFormData({...formData, autoPaid: e.target.checked})}
							className="w-5 h-5 text-indigo-600 rounded-lg border-gray-300 focus:ring-indigo-500"
						/>
						<div>
							<p className="text-sm font-bold text-gray-900">Auto-calculate total</p>
							<p className="text-[10px] text-gray-500 font-medium">Includes all unpaid services automatically</p>
						</div>
					</label>
				</div>
			</Modal>
